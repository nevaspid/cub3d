...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   draw_primitive_bonus.c                             :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: oliove <oliove@student.42.fr>              +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/06/08 06:10:51 by oliove            #+#    #+#             */
/*   Updated: 2024/07/11 14:46:55 by oliove           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_circle(mlx_image_t *img, int center_x, int center_y, int radius,
		int color, int fill)
{
	int	x;
	int	y;
	...
	while (x <= y)
	{
		if (fill == 1)
		{
			draw_fill_circle(img, center_x - x, center_x + x, center_y + y,
				MY_BLACK);
			draw_fill_circle(img, center_x - x, center_x + x, center_y - y,
				MY_BLACK);
			draw_fill_circle(img, center_x - y, center_x + y, center_y + x,
				MY_BLACK);
			draw_fill_circle(img, center_x - y, center_x + y, center_y - x,
				MY_BLACK);
		}
...
void	draw_compass(t_display *display, t_compass *compass, t_player *player)
{
	...
	draw_circle(compass->img, compass->center_x, compass->center_y,
		compass->radius, MY_WHITE, 1);
	draw_circle(compass->img, compass->center_x, compass->center_y,
		compass->radius - 2, MY_BLACK, 0);
	draw_circle(compass->img, compass->center_x, compass->center_y,
		compass->radius - 1, MY_WHITE, 0);
